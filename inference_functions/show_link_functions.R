...
links <- data.frame("Pi" = rep(c(1, 0.5, 0), each = 11),
                    "alpha" = rep(ALPHAS, 3),
                    "bRF" = mapply(weights_rf, rep(c(1, 0.5, 0), each = 11), rep(ALPHAS, 3)),
                    "LASSO-D3S" = mapply(weights_lasso, rep(c(1, 0.5, 0), each = 11), rep(ALPHAS, 3)))

links %>%
  gather(key = "method", value = "weight", bRF, LASSO.D3S) %>%
