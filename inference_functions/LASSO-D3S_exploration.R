...
#' 
#' Il me semble que tracer les courbes 'Mean of edges PWM presence scores' en 
#' fonction de alpha, non pas pour différents niveaux de densité mais pour différents 
#' niveaux de robustesse (sans étape de p-vlue) avec l'indication de la densité 
#' du réseau (ou du degré entrant moyen) correspondant à chaque seuil permettrait 
#' de nous éclairer sur le choix seuil de robustesse.
#' 
                                   ...
                                   doRNG::"%dorng%"(foreach::foreach(target = genes, .combine = rbind, .inorder = T),
                                                    {
                                                      ...
                                                      if(score=="freq"){
                                                        ...
                                                        to_return <- to_return[to_return$importance >= robustness,]
                                                        to_return$pwm <- pwm_imputed[cbind(to_return$to, to_return$from)]
                                                        to_return <- to_return[c("from", 'to', 'pwm')]
...
#'
#' @return list of regulatory interactions weighted by p-value
LASSO.D3S_selection_rate <- function(counts, genes, tfs, alpha=0.25, 
                                     pwm_occurrence, int_pwm_noise = 0.1,
                                     nfolds.cv = 5, N = 50, 
                                                      ...
                                                      for(n in 1:N) {
                                                        ...
                                                        if(length(oob)>1){ #ensures a test set of sufficient size
                                                          ...
                                                          tryCatch(
                                                            ...
                                                            sel_rate <- sel_rate+length(selected_tfs)
                                                            i_l_min <- i_l_min+iLambdaMin
                                                            })
                                                        }
                                                      }
                                                      
                                                      data.frame(sel_rate = sel_rate/N,
                                                                 target = target,
                                                                 i_l_min = i_l_min/N)
...
#'
#' @return list of regulatory interactions weighted by p-value
LASSO.D3S_inference_CV <- function(counts, genes, tfs, alpha=0.25, 
                                   pwm_occurrence, 
