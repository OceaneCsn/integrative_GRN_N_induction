...
```{r}
# imports the optimal values of alpha of the proposed approach DIOgene
...
p_enrich <- phyper(q=567, m = length(names(alphas_rf[alphas_rf!=0])), 
                   n = length(genes) - length(names(alphas_rf[alphas_rf!=0])), 
                   k = length( names(alphas_lasso[alphas_lasso!=0])), lower.tail = F)
p_enrich
ggexport(venn, filename = "results/supp_figures/alpha_classes_intersection.pdf", width = 4, height = 4)
```
...
```{r}
load("results/rdata/gene_specific/gene_specific_edges_student.rdata")
# load("results/rdata/gene_specific/gene_specific_edges_true_sd_en.rdata")
...
degree_plot <- deg %>%
  ...
  geom_boxplot(width = 0.1, fill = "black", col = "white", outlier.alpha = 0)+stat_compare_means()+
  facet_nested_wrap(vars(model, alpha), nest_line = T, ncol = 6)+
  scale_fill_brewer(palette = "Accent")+ ylab("Log of degree") +
  theme_bw()+ theme(strip.background = element_blank(), legend.position = "top"); degree_plot
...
unique_regs <- setdiff(get_hubs(rf_nets)$top25 , 
        union(get_hubs(rf_nets_0)$top25,
...
ranks_rf <- get_hubs(rf_nets_0)$ranking+ ggtitle("weightedRF alpha = 0") + 
  ...
  theme(legend.position = "none")+
  get_hubs(rf_nets, unique_regs = c("HHO6", "BZIP53", "ABF2"))$ranking+ ggtitle("DIOgene weightedRF")+
...
```{r}
# regulators that were not in the top 25
unique_regs <- setdiff(get_hubs(lasso_nets)$top25 , 
...
ranks_lasso <- get_hubs(lasso_nets_0)$ranking+ ggtitle("weightedLASSO alpha = 0") +
  ...
  theme(legend.position = "none")+
  get_hubs(lasso_nets, unique_regs = unique_regs)$ranking+ ggtitle("DIOgene weightedLASSO")+
  theme(legend.position = "top", legend.text = element_text(size = 15))+
  plot_layout(guides = "collect") 
