...
ggexport(charact, width = 14, height = 12,
         filename = "results/supp_figures/genes_characteristics_TFBM_integration_en.pdf")
```
...
lasso_nets_0 <- names(edges)[str_detect(names(edges),"EN_0_trueData") ]
lasso_nets_0 <- edges[lasso_nets_0[str_detect(lasso_nets_0,"0.005") ]]

lasso_nets_1 <- names(edges)[str_detect(names(edges),"EN_1_trueData") ]
...
ranks_lasso <- get_hubs(lasso_nets_0)$ranking+ ggtitle("weightedEN alpha = 0") +
  theme(legend.position = "none")+
  get_hubs(lasso_nets_1)$ranking+ggtitle("weightedEN alpha = 1") +
  theme(legend.position = "none")+
  get_hubs(lasso_nets, unique_regs = unique_regs)$ranking+ ggtitle("weightedEN gene-specific alpha")+
  theme(legend.position = "top", legend.text = element_text(size = 15))+
  plot_layout(guides = "collect") 

ggexport(ranks_lasso, filename = "results/supp_figures/weightedEN_ranks_grns.pdf", width = 20, hight = 7)
