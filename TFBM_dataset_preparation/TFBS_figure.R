...
mot_promN <- ggplot(data.frame(counts), aes(x =Freq)) + 
  geom_histogram(fill = "#8497B0", alpha=1) + ggtitle("Number of distinct TFBM per promoter") +
...
cib_promN <- ggplot(data.frame(counts), aes(x =Freq)) + 
  geom_histogram(fill = "gray", alpha=1) + ggtitle("Number of distinct target TFBM per TF")+
