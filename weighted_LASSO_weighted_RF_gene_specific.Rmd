...
## Functions for computing EDI and MSE based on the stored inference results


The following graphics show the MSE and effective data integration behaviors as a function of $alpha$.

This chunk loads the saved results of the parametric exploration for both models on true and shuffled datasets, and defines the functions for visualization and computation of the optimal value of $alpha$ per gene.
...
draw_gene_effective_integration <- function(gene, prior=1, type = "rank", return = F){
...
get_opt_alpha_per_gene <- function(gene, type = "rank", dev = "mean",
  ...
  # returns the optimal value of alpha for this gene
...
How does the EDI vary when alpha increases?
...
edi_rf <- mcsapply(genes, draw_gene_effective_integration, 
...
# getting mean EDI values for all genes as a function of alpha
edi_lasso <- mcsapply(genes, draw_gene_effective_integration, 
                          return=T, mc.cores=40)
...
## Draw some gene examples to illustrate our gene-specific optimization :


### In weightedRF

Plots the results for the proposed gene-specific criterion for three gene examples :
...
Are the genes with $\alpha > 1$ the same between weightedLASSO and weightedRF?
...
# Precision and recall analyses

Validate an inferred GRN, with a given density, against an experimental gold standard (here, DAP-Seq).
...
Getting the final GRN models with a gene-specific value of $\alpha$
...
Differences between the two criteria for the lasso : 
