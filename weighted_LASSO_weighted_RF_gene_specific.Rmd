...
pr_curves <- data_val %>%
  ...
  theme_pubr()+ ggh4x::facet_nested_wrap(vars(model), nest_line = T)+
  geom_line(aes(y=mean_precision, x=mean_recall), size=2)+
  geom_label(aes(y=mean_precision, x=mean_recall), nudge_y = 0.02, fill = "white")+
  theme(strip.background = element_blank())+
  geom_hline(color = "#C55A11", yintercept = 0.331042, size= 1.5, show.legend = T) +
...
ggexport(plot_final, filename = "results/specific_grns_no_permutations_mean_sds.pdf", width = 11, height = 10)
...
```{r}
load("results/gene_specific_edges_true_sd.rdata")
