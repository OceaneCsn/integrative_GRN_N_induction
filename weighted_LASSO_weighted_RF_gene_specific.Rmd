...
Showing the precision of grns with max div method and D=0.01 
...
# Look at precision instead, in TARGET in order to remove indirect targets
...
get_precision <- function(model, density){
  df <- data.frame(precision_tga1 = double(0),
                   ...
                   n_targets = integer(0), model = character(0), 
  ...
  for(dataset in c("shuffled", "trueData")){
    ...
    df <- rbind.data.frame(df, data.frame(
          precision_tga1 =length(intersect(targs, tga1_tg))/length(targs),
          precision_tga4 =length(intersect(targs, tga4_tg))/length(targs),
          precision_vrn1 =length(intersect(targs, vrn1_tg))/length(targs),
          precision_hho5 =length(intersect(targs, hho5_tg))/length(targs),
          precision_hho3 =length(intersect(targs, hho3_tg))/length(targs),
          precision_div1 =length(intersect(targs, div1_tg))/length(targs),
...
```{r}
load("results/gene_specific_edges.rdata")
table(edges$RF_trueData_1_0.005$from)
TFs <- c("AT5G65210", "AT5G10030", "AT3G18990")

get_hubs <- function(net){
  hubs <- names(table(net$from)[order(table(net$from), decreasing = T)])[1:15]
...
get_hubs(edges$RF_trueData_1_0.005)
get_hubs(edges$RF_trueData_2_0.005)
get_hubs(edges$RF_trueData_3_0.005)
get_hubs(edges$LASSO_trueData_1_0.005)
get_hubs(edges$LASSO_trueData_2_0.005)
get_hubs(edges$LASSO_trueData_3_0.005)
...
```{r}
net_rf <- edges$RF_trueData_1_0.005[]
net_lasso <- edges$LASSO_trueData_1_0.005[]
