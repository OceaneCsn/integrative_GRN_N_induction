...
looking for new profiles (increasing mse but )

```{r}
lasso_1<- names(alphas_rf[alphas_rf==0])
subset <- sample(lasso_1, size = 10, replace = F)
...
```{r}
lasso_1<- names(alphas_rf[alphas_rf>0.5])
subset <- sample(lasso_1, size = 50, replace = F)
...
Figures for ISMB talk : 


```{r}
# mse increase (or AT5G64660)
compromise <- get_opt_alpha_per_gene("AT1G14720")+labs(subtitle = "AT1G14720")+
  get_opt_alpha_per_gene("AT5G16650")+labs(subtitle = "AT5G16650")
 

# mse decrese
ideal <- get_opt_alpha_per_gene("AT1G70790")+
  theme(legend.position = "right") + 
  labs(subtitle = "AT1G70790")


# mse increase_bad
bad <- get_opt_alpha_per_gene("AT1G56230") +labs(subtitle = "AT1G56230")+ 
  get_opt_alpha_per_gene("AT2G29990")+labs(subtitle = "AT2G29990")
...
eff_data <- draw_gene_effective_integration("AT1G14720")+
   theme(legend.position = "right") 
...
TAR DB gives us the targets of miRNAs. Are they enriched in alpha = 0 genes?


```{r}
mirnas <- readLines("data/miRNAs_targets.txt") 

mirnas <- mirnas[str_detect(mirnas, "AT")] |>
  str_split_fixed('\t', 6) |>
  as.data.frame() %>%
  select(V1, V2) %>%
  rename(mirna = V1, target = V2) %>%
  mutate(target = str_split_fixed(target, '\\.', 2)[,1])

cons_mirnas <- unique(mirnas$target)

mirnas <- readLines("data/miRNAs_targets_deg.txt") 
...
deg_mirnas <- unique(mirnas$target)

#all_mirnas_targets <- unique(c(deg_mirnas, cons_mirnas))
all_mirnas_targets <- unique(c(deg_mirnas))

length(intersect(genes, all_mirnas_targets))


no_int_rf <- names(alphas_rf[alphas_rf<0.5])
no_int_lasso <- names(alphas_lasso[alphas_lasso<0.5])
no_int_common <- intersect(no_int_rf, no_int_lasso)


# proportion in all genes
length(intersect(genes, all_mirnas_targets)) / length(genes)


# proportion in pos class
length(intersect(no_int_common, all_mirnas_targets)) / length(no_int_common)


# proportion in pos class
length(intersect(no_int_rf, all_mirnas_targets)) / length(no_int_rf)

# proportion in pos class
length(intersect(no_int_lasso, all_mirnas_targets)) / length(no_int_lasso)


# proportion
length(intersect(common, all_mirnas_targets))/length(common)

```

No result. Trying with a new database : 


```{r}
mirnas <- read.table("data/mirnas_dpmind.txt", h = T, sep = '\t') %>%
  select(Target_Acc.) 

all_mirnas_targets <- str_split_fixed(mirnas$Target_Acc., "\\.", 2)[,1]


length(intersect(genes, all_mirnas_targets))


no_int_rf <- names(alphas_rf[alphas_rf==0])
no_int_lasso <- names(alphas_lasso[alphas_lasso==0])
no_int_common <- intersect(no_int_rf, no_int_lasso)
...
# load("results/gene_specific_mse_true_sd.rdata")
# load("results/gene_specific_validation_true_sd.rdata")
...
# geom_label(aes(y=mean_precision, x=mean_recall), nudge_y = 0.02)+

pr_curves <- data_val %>%
  ggplot(aes(x=recall, y=precision, 
             label = alpha, color = interaction(dataset, alpha_type),
...
plot_final <- plot/ pr_curves+theme(legend.position = "none", 
  ...
  plot_layout(heights = c(1,1.5))+plot_annotation(title = "Shuffle SD")

ggexport(plot_final, filename = "results/specific_grns_shuffle_sd.pdf", width = 11, height = 10)
...
reshape2::melt()%>%
  separate(variable,
           into = c("model", "dataset",  "rep"),
           sep = "_") %>%
filter(model == "LASSO" & dataset == "trueData") %>%
group_by(dataset , rep) %>%
summarise(median_MSE = median(value, na.rm=T))%>%
...
reshape2::melt()%>%
  separate(variable,
           into = c("model", "dataset",  "rep"),
           sep = "_") %>%
filter(model == "RF"& dataset == "trueData") %>%
group_by(dataset , rep) %>%
summarise(median_MSE = median(value, na.rm=T))%>%
  ggplot(aes(x=dataset, y = median_MSE)) + 
geom_boxplot(size = 1, alpha = 0.5, outlier.alpha = 0, 
