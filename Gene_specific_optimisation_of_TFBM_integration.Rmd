...
figure <- (draw_gene_effective_integration(gene_benefit, mats = mats, prior = 1) +
  ...
  get_opt_alpha_per_gene(gene_no_benefit, mats = mats, lmses = lmses)+ 
  plot_annotation(tag_levels = c('a', '1')) + plot_layout(heights = c(0.8,1))

ggexport(figure, filename = "results/gene_examples_weightedRF.pdf", width = 10, height = 8)
...
ggexport(genes_removed, filename = "results/supp_figures/wRF_example_gene_lost.pdf", width = 10, height = 5)
...
# TFBM integration with min mse but not our approach
lost <- get_opt_alpha_per_gene("AT1G47395", mats = mats, lmses = lmses)+
  labs(subtitle = "AT1G47395")

# TFBM integration with our approach but not min MSE
