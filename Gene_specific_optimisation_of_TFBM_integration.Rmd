...
Results are a matrix of TF-target importances, GRN edges, MSE values, and validation against DAP-Seq. 
...
for(alpha in ALPHAS){ # exploring PWM integration strength
  for(rep in 1:nrep){ # exploring inherent variability
    ...
    mats[[paste0("EN_", as.character(alpha),  '_trueData_', rep)]] <- mat_lasso
    mats[[paste0("EN_", as.character(alpha),  '_shuffled_', rep)]] <- mat_lasso_perm
    
  }
}

save(mats, file = "results/rdata/weightedEN_importances_50rep.rdata")
...
save(edges, file = "results/rdata/weightedEN_edges_50rep.rdata")
save(lmses, file = "results/rdata/weightedEN_mse_50rep.rdata")
...
val_dap[, settings] <-
  str_split_fixed(val_dap$network_name, '_', length(settings))
save(val_dap, file = "results/rdata/weightedEN_validation_50rep.rdata")
...
# value of alpha per genes with student criterion:
stud_rf <- mcsapply(genes, get_opt_alpha_per_gene,mats = mats, 
                         lmses = lmses, dev = "student",
                          return_alpha=T, mc.cores=34, pval.adjust = "fdr")
save(stud_rf, file = "results/rdata/gene_specific/alpha_per_gene_weightedRF_student.rdata")


# value of alpha per genes:
alphas_rf <- mcsapply(genes, get_opt_alpha_per_gene,mats = mats, 
                         lmses = lmses,
                          return_alpha=T, mc.cores=34)
save(alphas_rf, file = "results/rdata/gene_specific/alpha_per_gene_weightedRF_true_sd.rdata")


# doing the same but with the value of alpha minimising the mse:
alphas_rf <- mcsapply(genes, get_opt_alpha_per_gene, metric = "min",mats = mats, 
                         lmses = lmses,
                          return_alpha=T, mc.cores=34)
...
# value of alpha per genes with student criterion:
stud_lasso <- mcsapply(genes, get_opt_alpha_per_gene,mats = mats, 
                         lmses = lmses, dev = "student", pval.adjust = "fdr",
                          return_alpha=T, mc.cores=34)
save(stud_lasso, file = "results/rdata/gene_specific/alpha_per_gene_weightedLASSO_student.rdata")
...
alphas_lasso <- mcsapply(genes, get_opt_alpha_per_gene, mats = mats, 
                         lmses = lmses,
                          return_alpha=T, mc.cores=34)
hist(alphas_lasso, breaks = 20)

save(alphas_lasso, file = "results/rdata/gene_specific/alpha_per_gene_weightedLASSO_true_sd.rdata")


# doing the same but with the value of alpha minimising the mse:
alphas_lasso <- mcsapply(genes, get_opt_alpha_per_gene, mats = mats, 
                         lmses = lmses, return_alpha=T, metric = "min", mc.cores=34)
save(alphas_lasso, file = "results/rdata/gene_specific/alpha_per_gene_weightedLASSO_min.rdata")
```


is student very different from our delta?
...
# tmp <- data.frame(stduent_rf = stud_rf, delta_rf = alphas_rf) %>%
#   ggplot(aes(x=delta, y = stud)) +
#   geom_point(alpha = 0.1)+
#   stat_cor()
# 
# ggsave(tmp, file = "tmp.pdf")

# 
# tmp <- data.frame(student_lasso_fdr = stud_lasso,
#                   delta_lasso = alphas_lasso, 
#                   student_rf_fdr = stud_rf,
#                   delta_rf = alphas_rf) %>%
#   reshape2::melt() %>%
#   ggplot(aes(x=value))+
#   geom_histogram()+facet_wrap(~variable, ncol = 2)
# ggsave(tmp, file = "student_vs_delta.pdf")
# 
# cor(alphas_lasso, stud_lasso);cor(alphas_rf, stud_rf)
# cor(alphas_lasso, alphas_rf );cor(stud_lasso, stud_rf)
```
...
ggexport(figure, filename = "results/main_figures/Figure_1_gene_examples_weightedRF.pdf", width = 10, height = 6)

```


```{r}
# example of gene lost by our approach but for which min MSE integrates TFBM
genes_removed <- get_opt_alpha_per_gene("AT3G04530", mats = mats, lmses = lmses)+ggtitle("AT3G04530")
ggexport(genes_removed, filename = "results/supp_figures/wRF_example_gene_lost.pdf", width = 3, height = 3)
```
...
for(rep in 1:nrep){ # exploring inherent variability
  ...
  mat_lasso <- weightedLASSO_inference(counts, genes, tfs,
                                   alpha = stud_lasso, N = 50,
                                   tf_expression_permutation = FALSE,
                                   mda_type = "shuffle", EN_param = 1,
                                   pwm_occurrence = pwm_occurrence,
                                   nCores = nCores)
...
save(mats, file = "results/rdata/gene_specific/gene_specific_grns_student.rdata")
...
save(lmses, file = "results/rdata/gene_specific/gene_specific_mse_student.rdata") 
save(edges, file = "results/rdata/gene_specific/gene_specific_edges_student.rdata") 
...
save(val_specific, file = "results/rdata/gene_specific/gene_specific_validation_student.rdata")
```
...
load(file = "results/rdata/gene_specific/gene_specific_grns_student.rdata")
...
save(optimized_grns, file = "inferred_GRN/optimized_GRN.rdata")
```
