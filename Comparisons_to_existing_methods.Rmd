...
comps_en_z <- (all_results %>%
    mutate(color = ifelse(method == "iRafNet" | method == "mLASSO_stars" , "ref",
      ...
      mutate(mean_precision =mean(precision),
             mean_recall = mean(recall)) %>%
              mutate(aupr_pos_x = ifelse(str_detect(method, "weighted"), 0.25, 0.15),
  ...
  ggh4x::facet_nested_wrap(vars(regression, paste("alpha =", alpha)), nest_line = T, ncol = 4)+
    ...
    geom_text(aes(y=aupr_pos_y, x=aupr_pos_x, label = paste("AUPR =",round(aupr, 2))), show.legend = T))/
  (all_results %>%
    mutate(color = ifelse(method == "iRafNet" | method == "mLASSO_stars" , "ref",
                          ifelse((method == "weightedLASSO" | method == "weightedEN" | method == "weightedRF") & 
                                   alpha == "DIOgene", "diogene", "weighted")),
  ...
  geom_point(aes(y=mse), size = 0.85, color = "black", show.legend = F)+
   geom_text(aes(label = round(mean_mse, 3), y=0.12, color = interaction(method, regression)), 
             nudge_x = 0.275, fill = "white", show.legend = F)+
  ggh4x::facet_nested_wrap(vars(regression, paste("alpha =", alpha)), 
                           nest_line = T, ncol = 4, scales = "free_x"));comps_en


ggexport(comps_en_z, filename = "results/comparisons_to_existing_methods/comparison_en.pdf", 
         width = 11, height = 8)
...
selection <- mat_mean_lasso_diogene != 0
to_add <- data.frame(model = "LASSO", alpha = "DIOgene", 
                     nb_tf = c(colSums(selection)),
                     mean_nb_tf = mean(colSums(selection)), 
                     sd_nb_tf = sd(colSums(selection)), strategy = "DIOgene")
results <- rbind.data.frame(results, to_add)
...
selection <- mat_mean_en_diogene != 0
...
model_complexity_hist <- results %>%
  ggplot(aes(x= nb_tf, color = model))+
  facet_wrap(model~alpha, ncol = 12)+
  geom_histogram();model_complexity_hist

model_complexity <- results %>%
  ggplot(aes(x=alpha, y = mean_nb_tf, color = model))+
  geom_point()+ylim(c(0,120)) + 
  geom_segment(aes(y = mean_nb_tf - sd_nb_tf, 
                   yend = mean_nb_tf + sd_nb_tf, xend = alpha))+
  geom_line()

compl <- model_complexity / model_complexity_hist

ggexport(compl, filename = "results/comparisons_to_existing_methods/linear_model_complexity.pdf", width = 15)
```
...
for(alpha in ALPHAS){
  ...
  grn_en <- weightedLASSO_network(mat_mean_en, density = 0.005, 
                                  pwm_occurrence = pwm_occurrence, 
                                  genes = genes, tfs = tfs)
  ...
  grn_lasso <- weightedLASSO_network(mat_mean_lasso, density = 0.005, 
                                  pwm_occurrence = pwm_occurrence, 
                                  genes = genes, tfs = tfs)
...
restricted_mse <- results %>%
  ggplot(aes(x=alpha, y = mse, color = model))+
  geom_boxplot()+ ylab("Median MSE")+ylim(c(0,1.1));restricted_mse


diversity <- results %>%
  group_by(alpha, model, strategy) %>%
  summarize(n_genes_in_grn = sum(mse != 1)) %>%
  ggplot(aes(x=alpha, y = n_genes_in_grn, color = model))+
  geom_point()+ ylab("Number of targets in the GRN");diversity

mse_plot <- diversity + restricted_mse

hist(mses, breaks = 50)
results_div <- results

results_div$genes <- rep(genes, 24)


ggexport(mse_plot, filename = "results/comparisons_to_existing_methods/linear_model_restricted_MSE.pdf", width = 15)

```

Mesures d'importance moins equitablement distribuées entre les gènes?


```{r, fig.width=15}
all_results <-results %>%
  rownames_to_column("genes") %>%
  mutate(genes = substr(genes, 1, nchar(genes[1]))) %>%
  full_join(results_div, by = c("genes", "model", "alpha", "strategy")) %>%
  mutate(is_included = mse!=1)

all_results%>%
  ggplot(aes(x=is_included, y = nb_tf, color = model))+
  geom_boxplot()+facet_wrap(model~alpha, ncol = 12)


save(all_results, file = "results/comparisons_to_existing_methods/linear_mse_complexity_results.rdata")
```

Tests for weightedEN


```{r}
importances_wLASSO <- weightedLASSO_inference(counts, genes, tfs, alpha = 0, 
                                                  N = 50, pwm_occurrence = pwm_occurrence, 
                                                  nCores = nCores, EN_param = 0.1)

importances_wLASSO_cheap <- weightedLASSO_inference(counts, genes, tfs, alpha = 0, 
                                                  N = 25, pwm_occurrence = pwm_occurrence, 
                                                  nCores = 45, EN_param = 0.1)
cor(importances_wLASSO["mse",], importances_wLASSO_cheap["mse",])


d <- data.frame(normal = c(importances_wLASSO[!str_detect(rownames(importances_wLASSO), "mse"),]), 
     cheap = c(importances_wLASSO_cheap[!str_detect(rownames(importances_wLASSO_cheap), "mse"),]))

d %>%
  ggplot(aes(x=normal, y=cheap))+
    geom_point(alpha = 0.5)+stat_cor()


eval <- evaluate_fully_connected(importances_wLASSO[tfs,], 
                                             pwm_occurrence = pwm_occurrence,
                              input_genes = genes, input_tfs = tfs, 
                                             validation = c("DAPSeq"),
                                             nCores =10) %>%
      mutate(alpha = 0, method = "weightedLASSO",
             mse = median(importances_wLASSO["mse",], na.rm = T),
             aupr = MESS::auc(recall, precision, type = 'linear')) 

eval_cheap <- evaluate_fully_connected(importances_wLASSO_cheap[tfs,], 
                                             pwm_occurrence = pwm_occurrence,
                              input_genes = genes, input_tfs = tfs, 
                                             validation = c("DAPSeq"),
                                             nCores =10) %>%
      mutate(alpha = 0, method = "weightedLASSO",
             mse = median(importances_wLASSO_cheap["mse",], na.rm = T),
             aupr = MESS::auc(recall, precision, type = 'linear')) 

```


Zoomer sur les courbes précision rappel pour les densités <) 0.1

lambda min alpha = 0.5 : [1] 0.2581444

lambda 1se alpha = 0.5 : [1] 0.2240055

lambda min alpha = 0.5 EN 0.5 : [1] 0.202

lambda 1se alpha = 0.5 EN 0.5 : [1] 0.187

lambda min alpha = 0 : [1] 0.245

lambda 1se alpha = 0 : [1] 0.208

lambda min alpha = 1 : [1] 0.245

lambda 1se alpha = 1 : [1] 0.248



Nb reg pour weightedLASSO (EN, LASSO, lambda min et lambda 1se) et 
