...
for(alpha in ALPHAS){ # exploring PWM integration strength
  for(rep in 1:nrep){ # exploring inherent variability
    ...
    mat_lasso1 <- weightedLASSO_inference(counts, genes, tfs,
                                     alpha = alpha, N = 50, 
...
save(edges, file = "results/lasso_perumtations_mse_all_genes_test.rdata")
...
grn <- weightedLASSO_network(mat_lasso1, density = 0.005, pwm_occurrence,
                          genes, tfs, decreasing = TRUE)
median(mat_lasso1["mse",])
mean(mat_lasso1["mse",])
summary(mat_lasso1["mse",])
