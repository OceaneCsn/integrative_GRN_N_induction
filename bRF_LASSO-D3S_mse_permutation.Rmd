...
########## tests ugly
nrt <- sample(lasso_1, size = 1)
draw_gene_effective_integration(nrt, prior = 1) +
...
lasso_1<- names(alphas_lasso_strict[alphas_lasso_strict==0.9])
...
spec_rf <- val_specific %>%
  ...
  ggplot(aes(x=dataset, y = precision, color = dataset, fill = dataset)) + 
   ggh4x::facet_nested_wrap(vars(density))+
  scale_color_manual(values = setNames(c("grey", "#70AD47"), c("shuffled", "trueData")))+
    scale_fill_manual(values = setNames(c("grey", "#70AD47"), c("shuffled", "trueData"))) +
  geom_hline(color = "#C55A11", yintercept = 0.331042, size= 1.5, show.legend = T) + 
...
precision_lasso+ ylim(c(0.2,0.45)) + theme(legend.position = 'none') + spec_lasso + 
  precision_rf + ylim(c(0.2,0.45))+theme(legend.position = 'none')+ggtitle("") + spec_rf + 
  plot_layout(guides = "collect", ncol = 4, widths = c(2,1,2,1))
