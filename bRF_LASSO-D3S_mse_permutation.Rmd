...
draw_gene_effective_integration <- function(gene, prior=1, type = "rank"){
  ...
  geom_ribbon(aes(ymin = mean_imp - sd_imp , 
    ...
    ylab("Rank of TFs with a PWM in the target promoter")+
...
get_opt_alpha_per_gene <- function(gene, type = "rank", return_cluster = F){
  ...
  padding = ifelse(alpha_opt<0.2, 10, -10)
  ...
  annotate("text", x=eff_opt+padding, y=max(true$mean_mse), 
           label=paste("Optimal data integration\nalpha =", alpha_opt), 
           angle=90, col = "#4670CD", size =5 )
  }
  else ggplot()
}
draw_gene_effective_integration(gene, prior = 1) +
  draw_gene_mse(gene)+
  get_opt_alpha_per_gene(gene)+
  plot_annotation(title = gene)

gene_no_benefit <- "AT1G30270"
gene_benefit <- "AT1G14720"
