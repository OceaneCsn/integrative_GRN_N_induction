...
if(strategy == "min_mse"){
  ...
  alphas_per_genes <- setNames(as.numeric(alphas_opt$alpha), 
                               rownames(alphas_opt))
...
for(model in c("rf", "lasso")){
  for(strat in c("min_mse", "max_div")){
    alphas_per_genes[paste0(model, "_", strat)] <- 
...
```{r}
draw_gene_mean_sd <- function(lmses, gene, title = NULL){
  lmses[gene, ] %>%
    gather() %>%
    separate(key,
             into = c("alpha", "rep", "MSEtype"),
