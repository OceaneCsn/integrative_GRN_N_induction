...
clusters_rank <- mcsapply(genes, get_opt_alpha_per_gene, type = "rank", return_cluster=T, mc.cores=34)
...
# with alpha not null?
save(clusters_rank, file = "results/clusters_eff_mse_bRF_100permutations_tolerent.rdata")
...
save(clusters_rank_lasso, file = "results/clusters_eff_mse_lasso_100permutations_tolerant.rdata")
```
...
load("results/clusters_eff_mse_lasso_100permutations_tolerant.rdata")
load("results/clusters_eff_mse_bRF_100permutations_tolerent.rdata")
...
save(networks, file = "results/gene_specific_alphas_grns_all_genes_eff_tolerant.rdata")

```

```{r, fig.width=10, eval=FALSE}
# load("results/gene_specific_alphas_grns_all_genes_eff.rdata")
nCores = 20
...
val <- evaluate_networks(networks, nCores = nCores,
                  input_genes = genes, validation = c("DAPSeq"))
...
save(val, file = "results/gene_specific_validation_tolerant.rdata")
...
# val_rf_pos <- val_dap
load("results/100_permutations_bRF_validation_dap_inf.rdata")
val_rf_all <- val_dap
...
plot_all_rf <- val_rf_all[(val_rf_all$alpha == "0"|val_rf_all$alpha == "1") & 
             as.numeric(val_rf_all$rep) <=20 & val_rf_all$density == 0.005 &
             val_rf_all$method == "bRF",] %>%
  ggplot(aes(x=dataset, color = dataset, y = precision)) + 
  ggh4x::facet_nested_wrap(vars(alpha), 
                           ncol = 8, nest_line = T) +
  geom_boxplot(outlier.alpha = 0, width = 0.2) + ggtitle("Global alpha") +
  ...
  stat_compare_means(label = "p.signif") + 
  ylim(c(0.2, 0.5)) + stat_compare_means(label = "p.signif")+
  theme_pubr(legend = "none") +ylab('')+
  geom_hline(yintercept = 0.331042) + 
  geom_boxplot(outlier.alpha = 0, width = 0.2) + ggtitle("Gene-specific alpha") +
  geom_jitter(width = 0.1)+xlab('')+
  plot_annotation(title = "Precision of 0.005 density networks for bRF ", 
                  subtitle = "GRNs of all genes") +
  plot_layout(widths = c(2,1));plot_all_rf
...
#              as.numeric(val_lasso_pos$rep) <=20 & val_lasso_pos$density == 0.005 &
#              val_lasso_pos$method == "LASSO",] %>%
#   ggplot(aes(x=dataset, color = dataset, y = precision)) + 
#   ggh4x::facet_nested_wrap(vars(alpha), 
#                            ncol = 8, nest_line = T) +
...
#   ylim(c(0.2, 0.5)) + theme_pubr(legend = "none")+
#   theme(strip.background = element_blank())+
#   geom_hline(yintercept = 0.3337577)+lab('')+
#   val %>%
#   filter(density == 0.005 & model == "lasso" & genesInNetwork == "positive")%>%
#   ggplot(aes(x=dataset, y = precision, color = dataset)) + 
#   stat_compare_means(label = "p.signif") + 
...
plot_all_lasso <- val_lasso_all[(val_lasso_all$alpha == "0"|val_lasso_all$alpha == "1") & 
             as.numeric(val_lasso_all$rep) <=20 & val_rf_all$density == 0.005 &
             val_lasso_all$method == "LASSO",] %>%
  ggplot(aes(x=dataset, color = dataset, y = precision)) + 
  ggh4x::facet_nested_wrap(vars(alpha), 
                           ncol = 8, nest_line = T) +
  geom_boxplot(outlier.alpha = 0, width = 0.2) + ggtitle("Global alpha") +
  geom_jitter(width = 0.1)+xlab('')+ylab('')+
  stat_compare_means(label = "p.signif")+
  geom_hline(yintercept = 0.331042) + 
  ylim(c(0.2, 0.5)) + theme_pubr(legend = "none")+
  theme(strip.background = element_blank())+
...
load("results/lasso_perumtations_mse_all_genes.rdata")
mse_data_lasso <- lmses[as.numeric(str_split_fixed(colnames(lmses), ' ', 3)[,2]) <=10] %>%
rownames_to_column('gene') %>%
  reshape2::melt()%>%
  separate(variable,
           into = c("alpha", "rep", "MSEtype"),
...
load("results/brf_perumtations_mse_all_genes_predict.rdata")
mse_data_rf <- lmses[as.numeric(str_split_fixed(colnames(lmses), ' ', 3)[,2]) <=10] %>%
rownames_to_column('gene') %>%
  reshape2::melt()%>%
  separate(variable,
           into = c("alpha", "rep", "MSEtype"),
...
mean_mses <- data.frame("dataset" = character(0), "model" = character(0), 
                        "alpha" = character(0), rep = integer(0), 
                        "mean_mse" = double(0), "median_mse" = double(0))

alphas <- list("0" = setNames(rep(0, length(genes)), genes), 
               "1" = setNames(rep(1, length(genes)), genes),
               "specific_lasso" = clusters_rank_lasso, 
               "specific_rf" = clusters_rank)

for(model in c("rf", "lasso")){
  for(alpha_strat in c("0","1","specific")){
    for(dataset in c("true_data", "shuffled")){
      
      if(model == "lasso") data <- mse_data_lasso
      if(model == "rf") data <- mse_data_rf
      
      if(alpha_strat == "specific") key = paste0(alpha_strat, '_', model)
      else key = alpha_strat
      
        mean_mses <- rbind.data.frame(mean_mses, data %>%
                           rename(mse = value) %>%
                            mutate(alpha = as.numeric(alpha))%>%
                            filter(MSEtype == dataset) %>%
                            filter(alpha == alphas[[key]][gene]) %>%
                            group_by(rep, MSEtype) %>%
                            summarize(mean_mse = mean(mse, na.rm=T),
                                      median_mse = median(mse, na.rm=T)) %>%
                             mutate(alpha = alpha_strat,
                                    model = model) %>%
                             rename(dataset = MSEtype))
    }
  }
...
mean_mses %>%
  ggplot(aes(x=dataset, y=mean_mse, color = dataset)) + 
  geom_boxplot(outlier.alpha = 0, width = 0.2) +
  geom_jitter(width = 0.1, size=0.8)+xlab('')+ylab('Mean(MSE/Var(gene))')+
  ggh4x::facet_nested_wrap(vars(model, alpha),nest_line = T, ncol = 6)+
...
global_alphas <-
  rbind.data.frame(val_lasso_all[(val_lasso_all$alpha == "0"|val_lasso_all$alpha == "1") & 
             as.numeric(val_lasso_all$rep) <=10 & val_lasso_all$density == 0.005 &
             val_lasso_all$method == "LASSO",],  val_rf_all[(val_rf_all$alpha == "0"|val_rf_all$alpha == "1") & 
             as.numeric(val_rf_all$rep) <=10 & val_rf_all$density == 0.005 &
             val_rf_all$method == "bRF",]) %>%
  mutate(dataset = str_replace(dataset, "Data", "_data"),
         method = str_replace(method, "LASSO", "lasso"),
         method = str_replace(method, "bRF", "rf")) %>%
  rename(model = method) %>%
  select(-network_name)

specific_alphas <- val %>%
  filter(density == 0.005 & genesInNetwork == "all")%>%
  select(precision, recall, model, dataset, rep, density) %>%
  mutate(rep = as.numeric(rep))%>%
  mutate(alpha = "specific",
         dataset = str_replace(dataset, "Data", "_data"))
  

precision_recall <- rbind.data.frame(global_alphas, specific_alphas)


# can not fix scale for variable only :(
full_join(mean_mses, precision_recall, by = c("alpha", "model", "dataset", "rep")) %>%
  reshape2::melt() %>%
  filter(!variable %in% c("median_mse")) %>%
  ggplot(aes(x=dataset, y=value, color = dataset)) + 
  geom_boxplot(outlier.alpha = 0, width = 0.2) +
  geom_jitter(width = 0.1, size=0.8)+xlab('')+ylab('Mean(MSE/Var(gene))')+
  ggh4x::facet_nested_wrap(vars(variable, model, alpha),nest_line = T, nrow=3, scales = "free_y")+
  stat_compare_means(hide.ns = T, label = "p.signif")+
  theme(strip.background = element_blank(), 
        axis.text.x = element_blank(), axis.ticks.x = element_blank()) 

plot_data <- full_join(mean_mses, precision_recall, by = c("alpha", "model", "dataset", "rep"))

precision <- plot_data %>%
  ggplot(aes(x=dataset, y=precision, color = dataset)) + 
  geom_boxplot(outlier.alpha = 0, width = 0.2) +
  geom_jitter(width = 0.1, size=0.5)+xlab('')+ylab('Precision')+
  ggh4x::facet_nested_wrap(vars(model, alpha),nest_line = T, nrow=1)+
  theme(strip.background = element_blank(), title = element_text(size = 8), 
        axis.text.x = element_blank(), axis.ticks.x = element_blank()) +
  geom_hline(yintercept = 0.3337577)+ 
  ggtitle("Precision against DAP-Seq interaction")


recall <- plot_data %>%
  ggplot(aes(x=dataset, y=recall, color = dataset)) + 
  geom_boxplot(outlier.alpha = 0, width = 0.2) +
  geom_jitter(width = 0.1, size=0.5)+xlab('')+ylab('Recall')+
  ggh4x::facet_nested_wrap(vars(model, alpha),nest_line = T, nrow=1)+
  theme(strip.background = element_blank(), title = element_text(size = 12), 
        axis.text.x = element_blank(), axis.ticks.x = element_blank())+ 
  ggtitle("Recall of 0.005 density networks against DAP-Seq interaction")

mse <- plot_data %>%
  ggplot(aes(x=dataset, y=mean_mse, color = dataset)) + 
  geom_boxplot(outlier.alpha = 0, width = 0.2) +
  geom_jitter(width = 0.1, size=0.5)+xlab('')+ylab('Mean(MSE/Var(gene))')+
  ggh4x::facet_nested_wrap(vars(model, alpha),nest_line = T, nrow=1)+
  theme(strip.background = element_blank(), title = element_text(size = 8), 
        legend.position = "none", 
        axis.text.x = element_blank(), axis.ticks.x = element_blank())+ 
  ggtitle("Prediction error on expression data") 
...
# Place of NRT2.1 in the two alpha specific networks

```{r}
grn_rf <- networks$`rf-max_div-0-all-trueData-8-0.005`
nrt2.1 <- "AT1G08090"
get_gene_information(grn_rf[grn_rf$to == nrt2.1,]$from, organism = "Arabidopsis thaliana")

mean(grn_lasso$pwm)
grn_lasso <- networks$`lasso-max_div-0-all-trueData-1-0.005`
get_gene_information(grn_lasso[grn_lasso$to == nrt2.1,]$from, organism = "Arabidopsis thaliana")


ranking_rf <- get_gene_information(names(sort(table(grn_rf$from), decreasing = T )[1:20]), 
                     organism = "Arabidopsis thaliana")

ranking_lasso <- get_gene_information(names(sort(table(grn_lasso$from), decreasing = T )[1:20]), 
                     organism = "Arabidopsis thaliana")

ranking_rf$pwm <- rownames(ranking_rf) %in% unique(pwm_prom$TF)

ranking_lasso$pwm <- rownames(ranking_lasso) %in% unique(pwm_prom$TF)
ranking_rf; ranking_lasso



grn_rf_0 <- rf_go$bRF_0_trueData_1_0.005

get_gene_information(names(sort(table(grn_rf_0$from), decreasing = T )[1:20]), 
                     organism = "Arabidopsis thaliana") %>%
  rownames_to_column("gene") %>%
  mutate(Motif_available = gene %in% unique(pwm_prom$TF))

common <- merge(grn_lasso, grn_rf, by = c("from", "to"))

get_gene_information(names(sort(table(common$from), decreasing = T )[1:15]), 
                     organism = "Arabidopsis thaliana")
...
rf_go <- edges[names(edges)[str_detect(names(edges), "trueData") &
               str_detect(names(edges), "bRF_0_|bRF_1_") &
               str_detect(names(edges), "0.005") &
               as.numeric(str_split_fixed(names(edges), '_', 5)[,4])<4]]
rf_go$bRF_0_trueData_1_0.005

load(file = "results/100_permutations_lasso_edges_mda_shuffle.rdata")

lasso_go <- edges[names(edges)[str_detect(names(edges), "trueData") &
               str_detect(names(edges), "LASSO_0_|LASSO_1_") &
               str_detect(names(edges), "0.005") &
               as.numeric(str_split_fixed(names(edges), '_', 5)[,4])<4]]


net_go <- networks[str_detect(names(networks), "0.005") &
                     str_detect(names(networks), "trueData") &
                     as.numeric(str_split_fixed(names(networks), '-',7 )[,6])<4]

global_alphas <- c(rf_go, lasso_go)
...
N_gsea <- networks_N_GSEA(net_go, N_go, nCores = 1)
N_gsea[,settings] <- str_split_fixed(N_gsea$network_name,'-',length(settings))

N_gsea_global <- networks_N_GSEA(global_alphas, N_go, nCores = 1)

settings <- c("model", "alpha", "dataset", "rep", "density")

N_gsea_global[,settings] <- str_split_fixed(N_gsea_global$network_name,'_',length(settings))
...
n_genes_gsea <- N_gsea %>%
  ...
  ggplot(aes(color = Pathway, x = model, y = enrichmentScore, 
             fill = Pathway)) +
  ggh4x::facet_nested_wrap(vars(density, Pathway), ncol = 10, nest_line = T) + 
   theme_pubr() +guides(color=guide_legend(nrow=5,byrow=TRUE))+ geom_point()+
  ...
  ggtitle("GSEA of N pathways in the inferred GRNs");n_genes_gsea


n_genes_gsea_global <- N_gsea_global %>%
  dplyr::select(-network_name) %>%
  mutate(model = str_replace(model, "bRF", "rf")) %>%
  mutate(
         density = paste0("D=", density),
         Pathway = str_replace(Description, "nitrogen compound", "N"),
         Pathway = str_replace(Pathway, "positive regulation of N metabolic process",
  ...
  ggplot(aes(color = Pathway, x = model, y = enrichmentScore, 
             fill = Pathway)) +
  ggh4x::facet_nested_wrap(vars(density, alpha, Pathway), ncol = 10, nest_line = T) + 
   theme_pubr() +guides(color=guide_legend(nrow=5,byrow=TRUE))+ geom_point()+
  theme(strip.background = element_blank(), axis.title.x = element_text(size = 22),
        title = element_text(size = 16), strip.text = element_text(size = 16), 
        legend.text = element_text(size = 12), axis.text = element_text(size = 12), 
        legend.position = 'none',legend.spacing.y = unit(0.15, 'cm')) + ylim(c(0.5,1))+
  ylab("Enrichment score") + 
  ggtitle("GSEA of N pathways in the inferred GRNs");n_genes_gsea

n_genes_gsea + n_genes_gsea_global
