...
mat_0 <- bRF_inference(counts=counts, genes= positive_genes_rf, tfs=tfs, alpha = 0, 
              tf_expression_permutation = F, pwm_occurrence = pwm_occurrence,
              nTrees = 2000, importance = "%IncMSE", nCores = 10)
grn_0 <- bRF_network(mat_0, 0.005, pwm_occurrence, positive_genes_rf, tfs)

mat_1 <- bRF_inference(counts=counts, genes= positive_genes_rf, tfs=tfs, alpha = 1, 
              tf_expression_permutation = F, pwm_occurrence = pwm_occurrence,
              nTrees = 2000, importance = "%IncMSE", nCores = 10)
grn_1 <- bRF_network(mat_1, 0.005, pwm_occurrence, positive_genes_rf, tfs)


evaluate_networks(list("expression" = grn_0, 
                       "max_integration" = grn_1), 
                  nCores = 2, input_genes = positive_genes_rf, 
                  input_tfs = tfs)
```
