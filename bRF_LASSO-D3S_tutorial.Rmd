...
```{r}
importances <- bRF_inference(counts, genes, tfs, alpha = 1, nTrees = 1000,
                             pwm_occurrence = pwm_occurrence, nCores = 25)

grn_bRF_prom <- bRF_network(importances, density = 0.005, pwm_occurrence, genes, tfs)
...
```{r}
lasso_imp <- LASSO.D3S_inferenceMDA(counts, genes, tfs, alpha = 0.8, N = 50,
                               pwm_occurrence = pwm_occurrence, nCores = 10)
summary(c(lasso_imp))
grn_lasso.d3s <- LASSO.D3S_network(lasso_imp, density = 0.005, pwm_occurrence, genes, tfs, decreasing = TRUE)
