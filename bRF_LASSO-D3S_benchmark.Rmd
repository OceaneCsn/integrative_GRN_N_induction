...
save(edges, file = "results/lasso_mda_0.rdata")
```
...
save(mses, file = "results/mse_lasso_mda_0.rdata")

```

```{r}
# load("results/mse_lasso_mda.rdata")
settings <- c("method", "alpha", "density", "rep")
...
ggexport(figure, filename = "results/figure3_lasso_mda_0.pdf", width = 14, height = 10)
```
...
# Links between p-values and variability
...
hist(pvalues)
lasso_imp[lasso_imp==0] <- NA


gene_vari <- data.frame(target = rownames(counts), sd = matrixStats::rowSds(counts),
                        mean = matrixStats::rowMeans2(counts), 
                        cv = matrixStats::rowSds(counts)/matrixStats::rowMeans2(counts))


# reshape2::melt(data.frame(pvalues)) %>%
#   tidyr::drop_na() %>%
#   dplyr::rename(target = variable, pvalue = value) %>%
#   full_join(gene_vari, by = "target") %>%
#   group_by(target) %>%
#   summarise(pvalue = median(pvalue, na.rm=T), 
#             mean = median(cv, na.rm=T),
#             cv = median(cv, na.rm=T),
#             sd = median(sd, na.rm=T)) %>%
#   ggplot(aes(x=cv, y=pvalue)) +
#   geom_point(size=1) + stat_cor(color = 'black', method = "spearman") + 
#   theme(legend.position = "none") 


imp <- bRF_inference(counts, genes, tfs, alpha = 0.5,
                               pwm_occurrence = pwm_occurrence, nCores = 10)

reshape2::melt(data.frame(lasso_imp)) %>%
  tidyr::drop_na() %>%
  dplyr::rename(target = variable, pvalue = value) %>%
  full_join(gene_vari, by = "target") %>%
  ggplot(aes(x=sd, y=pvalue, color = target)) +
  geom_point(alpha = 0.1, size=1) + stat_cor(color = 'black', method = "spearman") + 
  theme(legend.position = "none")+
  ggtitle("P-value en fonction du SD")+
reshape2::melt(data.frame(lasso_imp)) %>%
  tidyr::drop_na() %>%
  dplyr::rename(target = variable, pvalue = value) %>%
  full_join(gene_vari, by = "target") %>%
  ggplot(aes(x=cv, y=pvalue, color = target)) +
  geom_point(alpha = 0.1, size=1) + stat_cor(color = 'black', method = "spearman") + 
  theme(legend.position = "none") +
  ggtitle("P-value en fonction du CV") + ylim(c(0,20))+
reshape2::melt(data.frame(imp)) %>%
  tidyr::drop_na() %>%
  dplyr::rename(target = variable, mda = value) %>%
  full_join(gene_vari, by = "target") %>%
  ggplot(aes(x=sd, y=mda, color = target)) +
  geom_point(alpha = 0.1, size=1) + stat_cor(color = 'black', method = "spearman") + 
  theme(legend.position = "none")+
  ggtitle("MDA en fonction du SD")+
reshape2::melt(data.frame(imp)) %>%
  tidyr::drop_na() %>%
  dplyr::rename(target = variable, mda = value) %>%
  full_join(gene_vari, by = "target") %>%
  ggplot(aes(x=cv, y=mda, color = target)) +
  geom_point(alpha = 0.1, size=1) + stat_cor(color = 'black', method = "spearman") + 
  theme(legend.position = "none") +
  ggtitle("MDA en fonction du CV") + plot_layout(ncol=2)


grn_lasso.d3s %>%
  mutate(importance = lasso_imp[cbind(grn_lasso.d3s$from, grn_lasso.d3s$to)]) %>%
  dplyr::rename(target = to) %>%
  group_by(target) %>%
  full_join(gene_vari, by = "target") %>%
  summarise(importance = median(importance), cv = mean(cv), mean = mean(mean)) %>%
  ggplot(aes(x=cv, y=importance, color = target)) +
  geom_point(alpha = 1, size=1) + stat_cor(color = 'black', method = "spearman") + 
  theme(legend.position = "none") +
  ggtitle("MDA en fonction du CV") + ylim(c(0,20))
```
# Links between number of motifs and variability

```{r}
load("rdata/pwm_prom_jaspar_dap.rdata")
hist(table(pwm_prom$target))
nb_motifs <- data.frame(target = names(table(pwm_prom$target)), 
                        nb_motifs = table(pwm_prom$target))

nb_motifs %>%
  full_join(gene_vari, by = "target") %>%
  tidyr::drop_na()%>%
  ggplot(aes(y=nb_motifs.Freq, x=cv, color = target))+
  geom_point() + 
  theme(legend.position = "none") + 
  stat_cor(color = "black")+
  ylab("Number of motifs in promoter")
...
nb_motifs %>%
  ...
  stat_cor(color = "black")+
  ylab("Number of motifs in promoter")+
  ggtitle("Number of PWMs in promoter depending on gene CV")
```
...
mse_debiased <- get_MSE(grn_lasso.d3s, method = "LASSO-D3S", nCores = 10) 
mse_raw <- get_MSE(grn_lasso.d3s, method = "LASSO-D3S", nCores = 10) 
get_MSE_baseline(counts, unique(c(grn_lasso.d3s$from, grn_lasso.d3s$to)), nCores = 10)
