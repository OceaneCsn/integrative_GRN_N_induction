...
save(edges, file = "../PWM_models_data_article/networks_for_benchmark_alpha_density_oob_no_pert_rob_0.1.rdata")
```
...
ggexport(figure, filename = "results/figure3_oob_no_perturbation_0.1rob.pdf", width = 14, height = 10)
```


# Prediction error and networks topology

## Computing prediction error on test expression data

```{r}
#load(file = "../PWM_models_data_article/networks_for_benchmark_alpha_density_oob_no_pert_rob_0.1.rdata")

mses <- list()
for(network in names(edges)){
  mses[[network]] <- get_MSE(edges[[network]], method = str_split_fixed(network, '_', 2)[,1], nCores=52)
}
save(mses, file = "../PWM_models_data_article/mse.rdata")
...
figure <- mse_plot+nTFs+nRegs + plot_layout(widths = c(1,1, 1)) + plot_annotation(tag_levels = 'a');figure
ggexport(figure, filename = "results/figure4.pdf", width = 16, height = 5)
```
...
networks_N_genes <- function(edges_list, n_genes, nCores = 32){
...
N_rank_fav <- networks_N_genes(edges, n_genes_favorites, nCores = 40)
...
save(N_gsea, file = "../PWM_models_data_article/N_gsea.rdata")
      ...
      doRNG::"%dorng%"(foreach::foreach(network_name = names(edges_list), .combine = rbind),
                       {
                         ...
                         if(agi %in% nodes$id){
                           ...
                           if(!is.null(regs)){
                             infos <- get_gene_information(regs, organism = "Arabidopsis thaliana")
...
nrt2.1 <- NRT_regs(edges, "AT1G08090", nCores = 40)
...
nrt2.1_plot <- nrt2.1 %>%
  filter(regulators %in% get_gene_information(pwm_nrt2.1, organism = "Arabidopsis thaliana")$label) %>%
  select(-network_name) %>%
  mutate(alpha = as.numeric(alpha),
         method = str_replace(method, "Biased Random Forests", "bRF"),
         method = str_replace(method, "Differentially shrinked lasso SS", "LASSO-D3S"),
         ...
         knwon_pwm = ifelse(knwon_pwm==1, " PWM in promoter", "No PWM")) %>%
  ggplot(aes(fill = regulators, x = alpha, y = regulators)) +
  ggh4x::facet_nested_wrap(vars(method, knwon_pwm, density),drop = F,ncol = 8, nest_line = T) + 
  geom_tile(color = "grey", alpha = 0.4) + theme_pubr() +
  theme(strip.background = element_blank(), axis.title.x = element_text(size = 22),
        title = element_text(size = 16), strip.text = element_text(size = 16), 
        legend.text = element_text(size = 15), axis.text = element_text(size = 12), 
