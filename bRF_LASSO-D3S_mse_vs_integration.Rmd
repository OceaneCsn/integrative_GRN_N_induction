...
draw_gene_effective_integration <- function(gene){
  ...
  data.frame(lapply(mats, function(mat){mean(mat[,gene][tfs_with_motif])})) %>%
...
draw_MSE_vs_gene_effective_integration <- function(gene){
  ...
  inte <- data.frame(lapply(mats, function(mat){mean(mat[,gene][tfs_with_motif])})) %>%
  gather(key = "setting", value = "summed_importance") %>%
  separate(setting, into = c("method", "alpha", "dataset", "rep"), sep = "_") %>%
  group_by(alpha, dataset) %>%
  mutate(mean_imp = mean(summed_importance),
  ...
  geom_ribbon(aes(ymin =lower , 
                  ymax = upper  ), 
                alpha = .4)+ 
  geom_point(alpha = 0.5, size = 0.5)+theme_pubr(legend = "top") + geom_smooth(method = "loess", se = F)+
...
# 
# loess <- plot(all_data$summed_importance,
#               loess.sd(all_data$summed_importance, all_data$value)$sd)
# 
...
# 
# gene <- "AT3G49780"
# bad_rf_rank <- draw_gene_effective_integration(gene)+draw_gene_mean_sd(gene)+draw_MSE_vs_gene_effective_integration(gene)
# gene <- "AT3G17609"
# good_rf_rank <- draw_gene_effective_integration(gene)+draw_gene_mean_sd(gene)+draw_MSE_vs_gene_effective_integration(gene)
# 
# 
# bad_rf_rank/bad_rf_imp
# good_rf_rank/good_rf_imp
```
