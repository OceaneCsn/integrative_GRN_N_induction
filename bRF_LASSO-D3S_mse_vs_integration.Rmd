---
title: "Integrative GRN inference : permuting the expression and motifs of TFs: effect on expression prediction"
output: 
  html_document:
    df_print: paged
    toc: true
    toc_float: true
    toc_depth: 2
    code_folding: hide
---


```{r setup, include=FALSE}
knitr::opts_chunk$set(echo = TRUE, warning = F, message = F, fig.width = 15)

source('inference_functions/bRF.R')
source('inference_functions/LASSO-D3S.R')
source('inference_functions/evaluateNetwork.R')
source('inference_functions/MSE.R')
# library(ComplexHeatmap)
library(ggplot2)
library(tidyverse)
library(ggpubr)
library(patchwork)
library(ComplexHeatmap)
library(msir)
library(data.table)


```

This document demonstrates the use of the bRF and LASSO-D3S functions for integrative GRN inference.

Those functions infer the regulatory pathways of *Arabidopsis thaliana*'s roots in response to nitrate (N) induction from [Varala et al., 2018](https://www.ncbi.nlm.nih.gov/geo/query/acc.cgi?acc=GSE97500).

They use as inputs the expression profiles of N-responsive genes and TFBS information. 
Prior TFBS information was built by searching in the promoters of the N-responsive genes the PWM of the N-responsive regulators. 

# Data import

Import of the expression data and the N-responsive genes and regulators :

```{r}
load('rdata/inference_input_N_response_varala.rdata')
genes <- input_data$grouped_genes; length(genes)
tfs <- input_data$grouped_regressors; length(tfs)
counts <- input_data$counts; dim(counts)
load("rdata/pwm_occurrences_N_response_varala.rdata")
dim(pwm_occurrence)


ALPHAS <- seq(0,1, by = 0.1)

subset <- sample(genes, replace = F, size = 20)
#subset <- genes


load("results/brf_perumtations_mse_all_genes_predict.rdata")
load("results/100_permutations_bRF_importances_inf.rdata")
```



# Is the mse variation worsened by unmapping prior values and expression profiles of TFs?


## For Rfs

```{r, message=F}

draw_gene_mean_sd <- function(gene, title = NULL){
  lmses[gene, ] %>%
    gather() %>%
    separate(key,
             into = c("alpha", "rep", "MSEtype"),
             sep = " ") %>%
    group_by(alpha, MSEtype) %>%
    mutate(mean_mse = mean(value, na.rm = T),
           sd_mse = sd(value, na.rm = T)) %>%
    ggplot(aes(
      x = as.numeric(alpha),
      y = value,
      color = MSEtype,
      fill = MSEtype
    )) +ggtitle(paste(gene, title))+ylab("MSE/Var(gene)")+
    geom_ribbon(aes(ymin = mean_mse - sd_mse , 
                    ymax = mean_mse + sd_mse  ), 
                alpha = .4)  +theme_pubr(legend = "top")+
    geom_point(alpha = 0.1) + geom_smooth(se=F)+xlab("alpha")

}

pwm_imputed <- pwm_occurrence
pwm_imputed[is.na(pwm_imputed)] <- 0.5




draw_MSE_vs_gene_effective_integration <- function(gene, return_cluster = F){
  tfs_with_motif <- names(which(pwm_occurrence[gene,]==1))
  if(return_cluster & length(tfs_with_motif)  == 0)
    return("not saved")
  if(length(tfs_with_motif)==0)
    return((ggplot()+ ggtitle ("no motif in this gene")))
  inte <- data.frame(lapply(mats, function(mat){mean(rank(mat[,gene])[tfs_with_motif])})) %>%
  gather(key = "setting", value = "summed_importance") %>%
  separate(setting, into = c("method", "alpha", "dataset", "rep"), sep = "_") %>%
  group_by(alpha, dataset) %>%
  mutate(mean_imp = mean(summed_importance),
            sd_imp = sd(summed_importance),
         alpha = as.numeric(alpha))
  
  curves <- lmses[gene, ] %>%
    gather() %>%
    separate(key,
             into = c("alpha", "rep", "MSEtype"),
             sep = " ") %>%
    group_by(alpha, MSEtype) %>%
    rename(dataset = MSEtype)%>%
    mutate(mean_mse = mean(value, na.rm = T),
           sd_mse = sd(value, na.rm = T),
           alpha = as.numeric(alpha),
           dataset = str_replace(dataset, "true_data", "trueData")) %>%
    full_join(inte, by = c("alpha", "dataset", "rep")) %>%
    group_by(dataset) %>%
    dplyr::arrange(summed_importance) %>% 
    mutate(lower = loess.sd(x=summed_importance, y=value)$lower,
           upper = loess.sd(x=summed_importance, y=value)$upper) %>%
    mutate(lower = frollmean(lower, 1, fill = NA),
           upper = frollmean(upper, 1, fill = NA))
  
    true <- curves %>%
    filter(dataset == "trueData")
  shuff <- curves %>%
    filter(dataset != "trueData")
  
  true[paste0("shuffled_", c("mean_mse", "lower", "upper"))] <- shuff[c("mean_mse", "lower", "upper")] 
  dev <- true%>%
    mutate(deviation = ifelse(shuffled_lower - upper < 0 , 0, shuffled_lower - upper))
  
  cluster = "not saved"
  if(sum(dev$deviation > 0, na.rm = T) > 10) cluster = "class of interest"
  if(return_cluster) return(cluster)

  curves %>%
  ggplot(aes(y=value, x = summed_importance, color = dataset, fill = dataset))+
  geom_ribbon(aes(ymin =lower , 
                  ymax = upper  ), 
                alpha = .4)+ geom_point(alpha = 0.1, size = 0.5) +
  theme_pubr(legend = "top") + geom_smooth(method = "loess", se = F)+
    ylab("MSE/Var(gene)") + xlab("effective integration") + ggtitle(paste(gene, cluster))
}


draw_MSE_vs_gene_effective_integration_mean <- function(gene, type = "rank", return_cluster = F){
  tfs_with_motif <- names(which(pwm_occurrence[gene,]==1))
  if(type == "rank")
    data <- data.frame(lapply(mats, function(mat){mean(rank(mat[,gene])[tfs_with_motif])}))
  if(type == "imp")
    data <- data.frame(lapply(mats, function(mat){mean(mat[,gene][tfs_with_motif])}))
  
  if(return_cluster & length(tfs_with_motif)  == 0)
    return("not saved")
  if(length(tfs_with_motif)==0)
    return((ggplot()+ ggtitle ("no motif in this gene")))
  
  inte <- data %>%
  gather(key = "setting", value = "summed_importance") %>%
  separate(setting, into = c("method", "alpha", "dataset", "rep"), sep = "_") %>%
  group_by(alpha, dataset) %>%
  mutate(mean_imp = mean(summed_importance),
            sd_imp = sd(summed_importance),
         alpha = as.numeric(alpha))
  
  curves <- lmses[gene, ] %>%
    gather() %>%
    separate(key,
             into = c("alpha", "rep", "MSEtype"),
             sep = " ") %>%
    rename(dataset = MSEtype,)%>%
    mutate(alpha = as.numeric(alpha),
           dataset = str_replace(dataset, "true_data", "trueData"))%>%
    full_join(inte, by = c("alpha", "dataset", "rep")) %>%
    group_by(mean_imp, dataset) %>%
    mutate(mean_mse = mean(value, na.rm = T),
           sd_mse = sd(value, na.rm = T)) 
  curves %>%
  ggplot(aes(y=mean_, x = mean_imp, color = dataset, fill = dataset))+
  geom_ribbon(aes(ymin =mean_mse-sd_mse , 
                  ymax = mean_mse + sd_mse),  alpha = .4)+ 
    geom_point(alpha = 0.1, size = 0.5) +
    geom_line(aes(x=mean_imp, y = mean_mse)) + 
  theme_pubr(legend = "top") + 
    ylab("MSE/Var(gene)") + xlab("effective integration") + ggtitle(paste(gene))
}


draw_gene_effective_integration <- function(gene, prior=1, type = "rank"){
  tfs_with_motif <- names(which(pwm_imputed[gene,]== prior))
  if(type == "rank")
    data <- data.frame(lapply(mats, function(mat){mean(rank(mat[,gene])[tfs_with_motif])}))
  if(type == "imp")
    data <- data.frame(lapply(mats, function(mat){mean(mat[,gene][tfs_with_motif])}))
  plot <- data %>%
  gather(key = "setting", value = "summed_importance") %>%
  separate(setting, into = c("method", "alpha", "dataset", "rep"), sep = "_") %>%
  group_by(alpha, dataset) %>%
  mutate(mean_imp = mean(summed_importance),
            sd_imp = sd(summed_importance),
         alpha = as.numeric(alpha))%>%
  ggplot(aes(x=alpha, y = summed_importance, color = dataset, fill = dataset)) +
  geom_point(alpha = 0.1) + geom_smooth(se=F)+
  geom_ribbon(aes(ymin = mean_imp - sd_imp , 
                    ymax = mean_imp + sd_imp  ), 
                alpha = .4)  +theme_pubr(legend = "top")+
    ylab("effective data integration")+
    ggtitle(paste("prior:", prior, ',', length(tfs_with_motif), "TFs"), 
            subtitle = type)
  
  if(type == "rank")
    plot+ylim(c(0,200)) + 
    geom_hline(yintercept = 
                 (length(names(which(pwm_imputed[gene,]== 0.5)))+
                 length(names(which(pwm_imputed[gene,]== 0))))/2)
  else
    plot
}

for(gene in sample(genes, replace = F, size = 10)){
    print(draw_gene_effective_integration(gene)+draw_gene_mean_sd(gene)+
            draw_MSE_vs_gene_effective_integration_mean(gene)+
            draw_MSE_vs_gene_effective_integration_mean(gene, type = "imp")+
            plot_layout(ncol=4))
}

```


### Effective integration 


```{r, fig.height=5, fig.width=15}
for(gene in sample(genes, replace = F, size = 20)){
    print(draw_gene_effective_integration(gene, prior = 1)+
            draw_gene_effective_integration(gene, prior = 1, type = "imp")+
            draw_gene_effective_integration(gene, prior = 0.5)+
            draw_gene_effective_integration(gene, prior = 0.5, type = "imp")+
            draw_gene_effective_integration(gene, prior = 0)+
            draw_gene_effective_integration(gene, prior = 0, type = "imp")+ 
            plot_layout(ncol = 6) + plot_annotation(title = gene))
}
```



## On the class of interest:

```{r, eval=T}
load("results/clusters_mse_bRF_100permutations.rdata")
pos_genes <- names(which(clusters_rf==2))
# for(gene in sample(pos_genes,10, replace = F)){
#     print(draw_gene_effective_integration(gene)+draw_gene_mean_sd(gene)+draw_MSE_vs_gene_effective_integration(gene))
# }

for(gene in sample(pos_genes, replace = F, size = 10)){
    print(draw_gene_effective_integration(gene)+draw_gene_mean_sd(gene)+
            draw_MSE_vs_gene_effective_integration(gene)+
            draw_MSE_vs_gene_effective_integration_mean(gene)+
            plot_layout(ncol=4))
}
```

### Effective integration 

```{r, fig.height=5, fig.width=15}
for(gene in sample(pos_genes, replace = F, size = 10)){
    print(draw_gene_effective_integration(gene, prior = 1)+
            draw_gene_effective_integration(gene, prior = 1, type = "imp")+
            draw_gene_effective_integration(gene, prior = 0.5)+
            draw_gene_effective_integration(gene, prior = 0.5, type = "imp")+
            draw_gene_effective_integration(gene, prior = 0)+
            draw_gene_effective_integration(gene, prior = 0, type = "imp")+ 
            plot_layout(ncol = 6) + plot_annotation(title = gene))
}
```

# lasso

```{r, eval=T}
load("results/lasso_perumtations_mse_all_genes.rdata")
load("results/100_permutations_lasso_importances_mda_shuffle.rdata")

for(gene in sample(genes, replace = F, size = 20)){
    print(draw_gene_effective_integration(gene)+draw_gene_mean_sd(gene)+
            draw_MSE_vs_gene_effective_integration(gene)+
            draw_MSE_vs_gene_effective_integration_mean(gene)+
            plot_layout(ncol=4))
}
```


### Effective integration 


```{r, fig.height=5, fig.width=15}
for(gene in sample(genes, replace = F, size = 20)){
    print(draw_gene_effective_integration(gene, prior = 1)+
            draw_gene_effective_integration(gene, prior = 1, type = "imp")+
            draw_gene_effective_integration(gene, prior = 0.5)+
            draw_gene_effective_integration(gene, prior = 0.5, type = "imp")+
            draw_gene_effective_integration(gene, prior = 0)+
            draw_gene_effective_integration(gene, prior = 0, type = "imp")+ 
            plot_layout(ncol = 6) + plot_annotation(title = gene))
}
```


## On the class of interest :


```{r, eval=T}
load("results/clusters_mse_lasso_100permutations.rdata")
pos_genes <- names(which(clusters_lasso==1))

for(gene in sample(pos_genes, replace = F, size = 10)){
    print(draw_gene_effective_integration(gene)+draw_gene_mean_sd(gene)+
            draw_MSE_vs_gene_effective_integration(gene)+
            draw_MSE_vs_gene_effective_integration_mean(gene)+
            plot_layout(ncol=4))
}

```

### Effective integration 


```{r, fig.height=5, fig.width=15}
for(gene in sample(pos_genes, replace = F, size = 10)){
    print(draw_gene_effective_integration(gene, prior = 1)+
            draw_gene_effective_integration(gene, prior = 1, type = "imp")+
            draw_gene_effective_integration(gene, prior = 0.5)+
            draw_gene_effective_integration(gene, prior = 0.5, type = "imp")+
            draw_gene_effective_integration(gene, prior = 0)+
            draw_gene_effective_integration(gene, prior = 0, type = "imp")+ 
            plot_layout(ncol = 6) + plot_annotation(title = gene))
}
```



Sophie : 


Par ailleurs, je me questionne sur la priorisation des arêtes entre les différents gènes cibles (qui sont donc issues de différents modèles de régression).

Je voudrais m'assurer que les arêtes que l'on priorise sont issues de 'bons' modèles (i.e. pour lesquels on a une baisse de l'erreur de prédiction par rapport à la baseline obtenue par la moyenne de l'expression).

Il me semble que ça devrait être le cas. En gros, si le modèle estimé est comparable au modèle baseline, il ne devrait pas y avoir de grand écart entre l'erreur du modèle estimé et l'erreur après 'shuffling' ou mise à 0.

Mais, est-ce qu'il te serait rapide/possible de représenter un nuage de points de l'importance calculée en fonction du ratio :

 (MSE_baseline - MSE_full)/MSE_baseline ?

Ce ratio donne une idée de la qualité du modèle 'full' (valeurs entre 0 et 1).

Cela nous permettrait de s'en assurer. 

```{r}
baselines <- get_MSE_baseline(counts, genes, nCores = 30)
baselines <- setNames(baselines, genes)

var_expr <- setNames(matrixStats::rowSds(counts)*matrixStats::rowSds(counts), genes)
colnames(lmses)
hist(log(baselines))

# for the RFs
load("results/brf_perumtations_mse_all_genes.rdata")
load("results/100_permutations_bRF_importances.rdata")


ratio <- lmses[as.numeric(str_split_fixed(colnames(lmses), ' ', 3)[,2]) ==1] %>%rownames_to_column('gene') %>%
  reshape2::melt()%>%
  separate(variable,
           into = c("alpha", "rep", "MSEtype"),
           sep = " ") %>%
  filter(MSEtype == "true_data") %>%
  mutate(baseline = baselines[gene],
         var = var_expr[gene]) %>%
  mutate(quality_ratio = 1-value)

plot_alpha_0_rf <- data.frame(mats$bRF_0_trueData_1) %>%
  rownames_to_column("gene") %>%
  reshape2::melt() %>%
  full_join(ratio, by = "gene") %>%
  ggplot(aes(x=quality_ratio, y = value.x)) +
  geom_point(alpha = 0.1)+xlim(c(0,1)) + ggtitle("alpha = 0, RFs")+ ylab("importance")

plot_alpha_1_rf <-data.frame(mats$bRF_1_trueData_1) %>%
  rownames_to_column("gene") %>%
  reshape2::melt() %>%
  full_join(ratio, by = "gene") %>%
  ggplot(aes(x=quality_ratio, y = value.x)) +
  geom_point(alpha = 0.1)+xlim(c(0,1)) + ggtitle("alpha = 1, RFs") + ylab("importance")


plot_alpha_0_rf + plot_alpha_1_rf
```



```{r, fig.width=10, fig.height=10}


load("results/lasso_perumtations_mse_all_genes.rdata")
load("results/100_permutations_lasso_importances_mda_shuffle.rdata")
ratio <- lmses[as.numeric(str_split_fixed(colnames(lmses), ' ', 3)[,2]) ==1] %>%rownames_to_column('gene') %>%
  reshape2::melt()%>%
  separate(variable,
           into = c("alpha", "rep", "MSEtype"),
           sep = " ") %>%
  filter(MSEtype == "true_data") %>%
  mutate(baseline = baselines[gene],
         var = var_expr[gene]) %>%
  mutate(quality_ratio = 1-value)


plot_alpha_0_lasso <- data.frame(mats$LASSO_0_trueData_1) %>%
  rownames_to_column("gene") %>%
  reshape2::melt() %>%
  full_join(ratio, by = "gene") %>%
  ggplot(aes(x=quality_ratio, y = value.x)) +
  geom_point(alpha = 0.005)+xlim(c(0,1)) + ggtitle("alpha = 0, lasso")+ ylab("importance")

plot_alpha_1_lasso <-data.frame(mats$LASSO_1_trueData_1) %>%
  rownames_to_column("gene") %>%
  reshape2::melt() %>%
  full_join(ratio, by = "gene") %>%
  ggplot(aes(x=quality_ratio, y = value.x)) +
  geom_point(alpha = 0.005)+xlim(c(0,1)) + ggtitle("alpha = 1, lasso") + ylab("importance")
(plot_alpha_0_lasso +plot_alpha_1_lasso) / 
(plot_alpha_0_rf + plot_alpha_1_rf)
```

## MSE of RFs: why higher?

Maybe trees should be pruned

```{r}
mses <- bRF_inference_MSE(counts, genes, tfs, alpha = 0, pwm_occurrence = pwm_occurrence, nTrees = 1000, nCores = 20)
mean(mses)
```


## Importance metric of LASSO or RFs?

```{r}
mat_lasso <- LASSO.D3S_inference_stableMDA(counts, genes, tfs,
                                     alpha = alpha, N = 100, robustness = 0.2,
                                     int_pwm_noise = 0, mda_type = "shuffle",
                                     pwm_occurrence = pwm_occurrence,
                                     nCores = 40)

plot_alpha_0.5_lasso <- data.frame(mat_lasso) %>%
  rownames_to_column("gene") %>%
  reshape2::melt() %>%
  full_join(ratio, by = "gene") %>%
  ggplot(aes(x=quality_ratio, y = value.x)) +
  geom_hex() + ggtitle("alpha = 0.5, lasso")+ ylab("importance")



mat_rf <- bRF_inference(counts, genes, tfs, nTrees = 1000,
                            alpha = 1,
                            pwm_occurrence = pwm_occurrence,
                            nCores = nCores,
                            importance = "%IncMSE")

data.frame(mat_rf) %>%
  rownames_to_column("gene") %>%
  reshape2::melt() %>%
  full_join(ratio, by = "gene") %>%
  ggplot(aes(x=quality_ratio, y = value.x)) +
  geom_hex() + ggtitle("alpha = 0.5, rf")+ ylab("importance")


 grn <- bRF_network(mat_rf, density = 0.005, pwm_occurrence, genes, tfs)
 evaluate_network(grn, input_genes = genes, input_tfs = tfs, validation = c("DAPSeq"))
```


