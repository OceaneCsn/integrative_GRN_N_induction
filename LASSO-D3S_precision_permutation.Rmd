...
for(alpha in ALPHAS){ # exploring PWM integration strength
  for(rep in 1:nrep){ # exploring inherent variability
    ...
    mat_lasso <- LASSO.D3S_inference_stableMDA(counts, genes, tfs,
                                     alpha = alpha, N = 100, robustness = 0.8,
    ...
    mat_lasso_perm <- LASSO.D3S_inference_stableMDA(counts, genes, tfs,
                                     alpha = alpha, N = 100, robustness = 0.1,
...
save(mats, file = "results/100_permutations_lasso_importances_mda_shuffle_0.1.rdata")
```
...
for(name in names(mats)){ # exploring importance threshold stringency
      for(density in densities){
        ...
        LASSO.D3S_network(mats[[name]], density = density, pwm_occurrence, 
                          genes, tfs, decreasing = TRUE)
      }
}

save(edges, file = "results/100_permutations_lasso_edges_mda_shuffle_0.1.rdata")
...
save(edges, file = "results/100_permutations_lasso_edges_positive_same_density_mda_shuffle_0.1.rdata")
```



```{r, eval=FALSE}
load( "results/100_permutations_lasso_importances_mda_shuffle_0.1.rdata")
...
color_palette = c("#C12131", "#EC5D2F", "#FE945C" )
...
```{r, fig.width=10}
# load("results/100_permutations_lasso_edges_mda_shuffle.rdata")
...
save(val_dap, file = "results/100_permutations_lasso_validation_dap_mda_shuffle_0.1.rdata")
...
draw_precision_recall <- function(validation, data_type, prior_validation = 0.331042){
  data_val <- validation %>%
...
precision <- data_val %>%
  ...
  theme(
    ...
    axis.text = element_text(size = 12),
    legend.position = 'top'
  )+geom_hline(yintercept = prior_validation)
...
load("results/100_permutations_lasso_validation_.rdata")
...
draw_precision_recall(val_dap, "DAP-Seq, positive genes", prior_validation = 0.3337577)


load("results/100_permutations_bRF_validation_dap_cluster_pos.rdata")
draw_precision_recall(val_dap, "DAP-Seq, positive genes", prior_validation = 0.3547275)
